...
- `PDFCompressRunner` has no "recompress embedded images in place" API to reach for in PDFKit, so it re-renders each page to a JPEG at the chosen quality and rebuilds the document from those images. That trades vector/text pages for raster ones — a deliberate choice for a "make this smaller" tool, not meant for text-searchable archival compression.
- `ImageToPDFRunner` (the native replacement for the old `append_image_to_pdf.py`) makes overwriting the source PDF structurally impossible: its initializer throws if `existingPDFURL` and `outputURL` would resolve to the same file, rather than relying on callers to remember to pass a different path. The old script silently overwrote its input in place.
